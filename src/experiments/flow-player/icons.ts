// ============================================================
// Inline SVG icons used by the control bar. All SVGs use
// fill="currentColor" / stroke="currentColor" so the parent's CSS
// color drives them — that's how the accent variable propagates.
// 24×24 viewBox throughout for consistent sizing.
// ============================================================
...
// Big centered play button shown on the slot before playback. Clean
// translucent circle (no white border ring) + play triangle. The
// triangle uses fill="currentColor" so the boot script can tint it
// from data-accent-color (or, when set, data-thumb-color via CSS var
// inheritance — but practically: accent).
//
// Triangle geometry: viewBox is 64×64. Vertices at (25, 20), (25, 44),
// (46, 32) — the geometric centroid lands on x=32, the viewBox center,
// so the triangle reads as visually centered without an offset hack.
export const PLAY_TRIANGLE_BIG = `<svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" width="72" height="72" aria-hidden="true">
  <circle cx="32" cy="32" r="32" fill="rgba(0,0,0,0.6)"/>
  <polygon points="25,20 25,44 46,32" fill="currentColor"/>
</svg>`;
