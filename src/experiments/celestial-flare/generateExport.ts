// ============================================================
// Celestial Flare — Inline IIFE generator for Webflow JSON export
...
const EXPORT_KEYS = new Set([
  ...
  'warpStrength', 'warpScale', 'warpSpeed', 'waveIntensity',
...
function buildDialConfigBlock(dialConfig: DialConfig): string {
  ...
  for (const [folderName, controls] of Object.entries(dialConfig)) {
    ...
    for (const [key, value] of Object.entries(controls)) {
      ...
      // Skip object-type entries (custom slider configs like { type: 'ub-2', ... })
    }
...
export function generateExport(options: GenerateExportOptions): string {
  const {
    params,
    dialConfig,
    slug = 'celestial-flare',
    experimentTitle = 'Celestial Flare',
    version = 'v1',
  } = options;
...
(function() {
  ...
  function render() {
    ...
    gl.uniform1f(U.u_waveIntensity, P.waveIntensity);
    ...
    gl.uniform1f(U.u_circleEdge, P.circleEdge);
    gl.uniform1f(U.u_circleDensity, P.circleDensity);
    ...
    gl.uniform1f(U.u_flareLength, P.flareLength);
    gl.uniform1f(U.u_flareRainbow, P.flareRainbow);
    gl.uniform1f(U.u_flareCount, P.flareCount);
    gl.uniform1f(U.u_flareSpeed, P.flareSpeed);
    ...
    gl.uniform1f(U.u_grainVariation, P.grainVariation);
