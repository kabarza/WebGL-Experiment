// ============================================================
// html-in-canvas 4 — WebGL2 + html-in-canvas beta API
...
function createHtmlScene(canvas: HTMLCanvasElement) {
  ...
  root.dataset.experimentCanvasHtml = 'html-in-canvas-4';
  heading.textContent = 'Canvas Meets DOM';
  copy.textContent =
    'Select this text. It lives inside a <canvas> as real DOM, piped through WebGL shaders in real time. Aurora drifts behind, grain settles on top.';
...
async function initGL(ctx: ExperimentGLContext): Promise<ExperimentInstance> {
  ...
  if (!('texElementImage2D' in gl) || !('requestPaint' in canvas)) {
    throw new Error(
      'html-in-canvas 4 needs Chrome Canary with chrome://flags/#canvas-draw-element enabled.',
    );
  }
  ...
  // Initialize texture with 1x1 placeholder so it has valid storage
  ...
  // ── html-in-canvas paint cycle (matches html-in-canvas-5) ──
  let textureReady = false;
  let pendingPaint = false;
  let lastPaintTick = -1;

  const handlePaint = () => {
    // Defensive: re-set layoutsubtree if it was stripped
    ...
    try {
      ...
      console.warn('[html-in-canvas-4] texElementImage2D error (retrying):', e);
    }
  ...
  // Double-rAF ensures at least one full layout pass has completed
  ...
  return {
    render(time: number) {
      ...
      gl.uniform1f(uniforms.u_pixelSize, params.pixelSize as number);
      gl.uniform1f(
        uniforms.u_glitchIntensity,
        params.glitchIntensity as number,
      );
...
export const htmlInCanvas4Experiment: Experiment = {
  meta,
  controls,
  initGL,
};
