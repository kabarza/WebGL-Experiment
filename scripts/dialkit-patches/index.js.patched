...
var DialStoreClass = class {
  ...
  isUBConfig(value) {
    return typeof value === "object" && value !== null && "type" in value && typeof value.type === "string" && value.type.startsWith("ub-");
  }
  isHexColor(value) {
...
function buildResolvedValues(config, flatValues, prefix) {
  ...
  for (const [key, configValue] of Object.entries(config)) {
    ...
    } else if (isUBConfig(configValue)) {
      result[key] = flatValues[path] ?? configValue.default ?? 0;
...
function UBControl({ variant, label, value, step, onChange }) {
  ...
  const [editing, setEditing] = useStateUB(false);
  const [inputVal, setInputVal] = useStateUB('');
  ...
  const labelRef = useRefUB(null);
  const valueRef = useRefUB(null);
  const rowRef = useRefUB(null);
  const dragState = useRefUB({ active: false, startX: 0, startVal: 0, accDx: 0 });
  const pointerDownPos = useRefUB(null);
  ...
  const rubberBandWidth = useTransformUB(rubberStretchPx, (stretch) => `calc(100% + ${Math.abs(stretch)}px)`);
  const rubberBandX = useTransformUB(rubberStretchPx, (stretch) => stretch < 0 ? stretch : 0);

  const computeRubberStretch = useCallbackUB((clientX, sign) => {
    const rect = wrapperRectRef.current;
    if (!rect) return 0;
    const distancePast = sign < 0 ? rect.left - clientX : clientX - rect.right;
  ...
  const computeDodge = (handlePct) => {
    ...
    const rightThresh = (trackW - 10 - valueW - 8) / trackW * 100;
    return handlePct < leftThresh || handlePct > rightThresh;
  };

  const springDots = [-0.75, -0.5, -0.25, 0.25, 0.5, 0.75];

  // ==========================================
  // SPRING SLIDER VARIANTS (ub-1, ub-4, ub-7, ub-8)
  // ==========================================
  if (variant === "ub-1" || variant === "ub-4" || variant === "ub-7" || variant === "ub-8") {
    const alwaysShow = variant === "ub-4" || variant === "ub-8";
    const hasAutoScroll = variant === "ub-7" || variant === "ub-8";
    const hasMomentum = variant === "ub-4";

    const [handleX, setHandleX] = useStateUB(0);
    const [autoDir, setAutoDir] = useStateUB(0);
    const autoRef = useRefUB(0);
    const autoSpeedRef = useRefUB(0);
    const velRef = useRefUB(0);
    const frameRef = useRefUB(0);
    const valRef = useRefUB(value); valRef.current = value;

    // Handle position as percentage for dodge calculation
    ...
    const onMove = (e) => {
      ...
      if (!isClickRef.current) {
        const rect = wrapperRectRef.current;
        const cx = dragState.current.startX, hw = dragState.current.accDx;
        const offset = Math.max(-hw, Math.min(hw, e.clientX - cx));
        setHandleX(offset);
    ...
    const onUp = () => {
      ...
      if (hasMomentum && !isClickRef.current) {
        ...
        const coast = () => {
          vel *= 0.92; hx *= 0.88;
          setHandleX(hx);
          valRef.current = rnd(valRef.current + vel);
          onChange(valRef.current);
          if (Math.abs(vel) > step * 0.01) frameRef.current = requestAnimationFrame(coast);
          else setHandleX(0);
        };
        if (Math.abs(vel) > step * 0.1) frameRef.current = requestAnimationFrame(coast);
        else setHandleX(0);
      } else {
        setHandleX(0);
      }
    ...
    return jsxUB("div", { ref: wrapperRef, className: "dialkit-slider-wrapper", children: jsxsUB(
      motionUB.div,
      {
        className: `ub-spring ${isActive ? 'ub-spring--active' : ''}`,
        ...
        style: { width: rubberBandWidth, x: rubberBandX, cursor: autoDir !== 0 ? (autoDir > 0 ? 'e-resize' : 'w-resize') : undefined },
        children: [
          jsxUB("div", { className: "ub-spring__notch" }),
          springDots.map((d, i) => jsxUB("div", { className: "ub-spring__dot", style: { left: `${50 + d * 50}%` } }, i)),
          jsxUB(motionUB.div, {
            className: "ub-spring__handle",
            style: { left: `max(5px, min(calc(100% - 8px), calc(50% + ${handleX}px - 1.5px)))`, y: "-50%" },
            animate: {
              opacity: handleOpacity,
  ...
  // ==========================================
  // SCRUB FIELD VARIANTS (ub-2, ub-5)
  // ==========================================
  if (variant === "ub-2" || variant === "ub-5") {
    const isDiscrete = variant === "ub-2";
    const ZONES = [{ label: '\u00D710', mult: 10 }, { label: '\u00D71', mult: 1 }, { label: '\u00D70.1', mult: 0.1 }, { label: '\u00D70.01', mult: 0.01 }];
    const [speedMult, setSpeedMult] = useStateUB(1);

    const onDown = (e) => {
      if (editing) return; e.preventDefault();
      e.currentTarget.setPointerCapture(e.pointerId);
      const rect = rowRef.current?.getBoundingClientRect();
      dragState.current = { active: true, startX: 0, startVal: value, accDx: isDiscrete ? (rect?.top ?? 0) : (rect ? rect.top + rect.height / 2 : 0) };
      setDragging(true); setSpeedMult(1);
    };
    ...
    const onMove = (e) => {
      ...
      let mult;
      if (isDiscrete) {
        const rowTop = dragState.current.accDx, rowH = 36, relY = e.clientY - rowTop, zone = 50;
        if (relY < -zone) mult = 10; else if (relY < rowH * 0.5) mult = 1; else if (relY < rowH + zone) mult = 0.1; else mult = 0.01;
      } else {
        const centerY = dragState.current.accDx;
        const dy = centerY - e.clientY;
        mult = Math.max(0.001, Math.min(1000, Math.pow(10, dy / 50)));
      }
      setSpeedMult(mult);
      dragState.current.startVal += e.movementX * step * mult;
      onChange(rnd(dragState.current.startVal));
    };

    const onUp = () => { dragState.current.active = false; setDragging(false); setSpeedMult(1); };

    const speedLabel = isDiscrete
      ? (ZONES.find(z => z.mult === speedMult)?.label ?? '\u00D71')
      : (speedMult >= 1 ? `\u00D7${speedMult.toFixed(speedMult >= 10 ? 0 : 1)}` : `\u00D7${speedMult.toFixed(speedMult >= 0.1 ? 2 : 3)}`);

    return jsxsUB("div", { ref: rowRef, className: `ub-zones ${isActive ? 'ub-zones--active' : ''}`,
  ...
  // ==========================================
  // RING SLIDER VARIANTS (ub-3, ub-6)
  // ==========================================
  if (variant === "ub-3" || variant === "ub-6") {
    const alwaysShowMarks = variant === "ub-6";
    const hasMomentum = variant === "ub-6";
    const [zoom, setZoom] = useStateUB(100);
    const velRef = useRefUB(0);
    const frameRef = useRefUB(0);
    const valRef = useRefUB(value); valRef.current = value;

    // Rubber band for tape
    const tapeRubber = useMotionValueUB(0);
    const tapeWidth = useTransformUB(tapeRubber, (s) => `calc(100% + ${Math.abs(s)}px)`);
    const tapeX = useTransformUB(tapeRubber, (s) => s < 0 ? s : 0);

    const niceStepFn = (range) => { const raw = range / 8; const mag = Math.pow(10, Math.floor(Math.log10(Math.abs(raw) || 1))); const r = raw / mag; return r <= 1.5 ? mag : r <= 3.5 ? 2 * mag : r <= 7.5 ? 5 * mag : 10 * mag; };
    const tickSz = niceStepFn(zoom); const halfR = zoom / 2;
    const wMin = value - halfR, wMax = value + halfR;
    const ticks = []; const tStart = Math.ceil(wMin / tickSz) * tickSz;
    for (let t = tStart; t <= wMax; t += tickSz) {
      const pct = ((t - wMin) / zoom) * 100;
      if (pct >= 0 && pct <= 100) ticks.push({ pct, major: Math.abs(t % (tickSz * 5)) < tickSz * 0.01 });
    }
    ...
    const onDown = (e) => {
      ...
      dragState.current = { active: true, startX: e.clientX, startVal: value, accDx: zoom };
      velRef.current = 0;
    };
    ...
    const onMove = (e) => {
      ...
      if (!isClickRef.current) {
        velRef.current = -e.movementX;
        const w = wrapperRef.current?.offsetWidth || 200;
        onChange(rnd(dragState.current.startVal - (dx / w) * dragState.current.accDx));
      }
    };
    ...
    const onUp = () => {
      ...
      if (hasMomentum && !isClickRef.current) {
        let vel = velRef.current * (zoom / (wrapperRef.current?.offsetWidth || 200));
        const coast = () => {
          vel *= 0.94;
          valRef.current = rnd(valRef.current + vel);
          onChange(valRef.current);
    ...
    const onWheel = (e) => {
      if (editing) return; e.preventDefault();
      if (e.shiftKey) setZoom(z => Math.max(0.1, Math.min(100000, z * (e.deltaY > 0 ? 1.3 : 0.77))));
      else onChange(rnd(value + step * (e.deltaY > 0 ? 1 : -1)));
    };

    return jsxUB("div", { ref: wrapperRef, className: "dialkit-slider-wrapper", children: jsxsUB(
      motionUB.div,
      {
        className: `ub-tape ${alwaysShowMarks ? 'ub-tape--always-marks' : ''} ${isActive ? 'ub-tape--active' : ''}`,
        onPointerDown: onDown,
        onPointerMove: onMove,
        onPointerUp: onUp,
        onPointerCancel: onUp,
        ...
        onMouseEnter: () => setHovered(true),
        onMouseLeave: () => setHovered(false),
        style: { width: tapeWidth, x: tapeX },
        children: [
          jsxUB("div", { className: "dialkit-slider-hashmarks", children: ticks.map((t, i) =>
            jsxUB("div", { className: `dialkit-slider-hashmark ${t.major ? 'ub-tape__tick--major' : ''}`, style: { left: `${t.pct}%` } }, i)
          ) }),
